namespace Cognito.Test.API.Core.Models;
...
public class Result
{
    ...
    public static Result Fail<T>(string error)
    {
        return new Result<T>(default(T), false, error);
    }
    
    public static Result Fail<T>(IEnumerable<string> error)
    {
        return new Result<T>(default(T), false, error);
    }
...
public class Result<T> : Result
{
    ...
    protected internal Result(T value, bool success, string error) : base(success, new List<string>{error})
    {
        Value = value;
    }
